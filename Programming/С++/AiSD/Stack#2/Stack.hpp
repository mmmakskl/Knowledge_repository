...
public:
    ...
    template<typename U>
    friend std::ostream& operator<<(std::ostream& out, const StackArray<U>& stack);
};
...
template <typename T>
inline std::size_t StackArray<T>::getTop()
{
    return top_;
}
...
template <typename T>
inline std::ostream &operator<<(std::ostream &out, StackArray<T>& stack)
{
    std::size_t top = stack.getTop();
    if (top == 0)
    {
